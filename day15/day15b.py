...
seed = np.array(input_lines, dtype=int)
def tile(count):
    ret = seed.copy() + (count * np.ones_like(seed))
    return np.where(ret > 9, ret % 9, ret)

maze_arr = np.vstack(
        [np.hstack([seed,    tile(1), tile(2), tile(3), tile(4)]),
         np.hstack([tile(1), tile(2), tile(3), tile(4), tile(5)]),
         np.hstack([tile(2), tile(3), tile(4), tile(5), tile(6)]),
         np.hstack([tile(3), tile(4), tile(5), tile(6), tile(7)]),
         np.hstack([tile(4), tile(5), tile(6), tile(7), tile(8)])] )
